...
bool UniverseTest_SystemGenerationDeterminism(FUniverseTestResult& Result)
{
    ...
    int64 SectorY = 0;
    int64 SectorZ = 0;
    UTEST_TRUE(Result, FindPopulatedSector(Hierarchy, SectorX, SectorY, SectorZ));

    FStarSystemDescriptor First;
    UTEST_TRUE(Result, FStarSystemGenerator::GenerateSystem(Hierarchy, SectorX, SectorY, SectorZ, 0, First));
    UTEST_TRUE(Result, First.IsValid());

    // Immediate repeat.
    FStarSystemDescriptor Second;
    UTEST_TRUE(Result, FStarSystemGenerator::GenerateSystem(Hierarchy, SectorX, SectorY, SectorZ, 0, Second));
    UTEST_EQ_UINT(Result, Second.GetContentHash(), First.GetContentHash());
    ...
    UTEST_TRUE(Result, Generated > 0);

    FStarSystemDescriptor Third;
    UTEST_TRUE(Result, FStarSystemGenerator::GenerateSystem(Hierarchy, SectorX, SectorY, SectorZ, 0, Third));
    UTEST_EQ_UINT(Result, Third.GetContentHash(), First.GetContentHash());

    // Field-level comparison as well, so a failure says which field diverged
    // rather than only that the hash differs.
    UTEST_TRUE(Result, Third.Id == First.Id);
    UTEST_TRUE(Result, Third.Position == First.Position);
    UTEST_TRUE(Result, Third.Name == First.Name);
    UTEST_EQ_INT(Result, static_cast<int32>(Third.Star.Class), static_cast<int32>(First.Star.Class));
    UTEST_EQ_DOUBLE_EXACT(Result, Third.Star.MassSolar, First.Star.MassSolar);
    UTEST_EQ_DOUBLE_EXACT(Result, Third.Star.LuminositySolar, First.Star.LuminositySolar);
    UTEST_EQ_INT(Result, Third.Planets.Num(), First.Planets.Num());
    ...
    for (int32 Index = 0; Index < First.Planets.Num(); ++Index)
    {
        ...
        UTEST_TRUE(Result, Third.Planets[Index].Name == First.Planets[Index].Name);
    }
        ...
        for (int64 Y = 0; Y < 25; ++Y)
        {
            ...
            if (AlphaCount > 0 && BetaCount > 0)
            {
                 ...
                 && FStarSystemGenerator::GenerateSystem(BetaHierarchy, X, Y, 0, 0, Beta))
                {
                    ...
                    UTEST_TRUE(Result, Alpha.Id == Beta.Id);
                }
            }
        }
    }

    UTEST_TRUE(Result, Compared > 0);
    UTEST_EQ_INT(Result, Differed, Compared);      // every comparison differed
    UTEST_TRUE(Result, PopulationDiffered > 0);    // even the star map differs

    // And a third seed differs from both.
    const FUniverseSeedHierarchy GammaHierarchy = MakeHierarchy(TEXT("gamma"));
    UTEST_TRUE(Result, GammaHierarchy.GetUniverseSeedValue() != AlphaHierarchy.GetUniverseSeedValue());
    UTEST_TRUE(Result, GammaHierarchy.GetUniverseSeedValue() != BetaHierarchy.GetUniverseSeedValue());
...
bool UniverseTest_SystemGenerationOrderIndependence(FUniverseTestResult& Result)
{
    ...
    UTEST_TRUE(Result, Addresses.Num() > 10);
    ...
    for (int32 Index = 0; Index < Addresses.Num(); ++Index)
    {
        ...
        UTEST_TRUE(Result, FStarSystemGenerator::GenerateSystem(Hierarchy, Addresses[Index], System));
    ...
    for (int32 Index = 0; Index < Addresses.Num(); ++Index)
    {
        UTEST_EQ_UINT(Result, Backward[Index], Forward[Index]);
    }
...
bool UniverseTest_SystemIdStabilityAndSerialization(FUniverseTestResult& Result)
{
    ...
    UTEST_TRUE(Result, A == B);
    UTEST_EQ_UINT(Result, A.Hash, B.Hash);
    UTEST_TRUE(Result, A.IsValid());

    // Distinct addresses, including permutations and the origin edge.
    UTEST_TRUE(Result, A != FStarSystemGenerator::MakeSystemId(-17, 4, 2000000, 0));
    UTEST_TRUE(Result, A != FStarSystemGenerator::MakeSystemId(4, -17, 2000000, 1));
    UTEST_TRUE(Result,
    ...
    UTEST_TRUE(Result, FStarSystemGenerator::MakeSystemId(0, 0, 0, 0).IsValid());

    // Round trip.
    {
        FUniverseByteWriter Writer;
        A.Serialize(Writer);
        UTEST_EQ_INT(Result, Writer.Num(), FUniverseSystemId::SerializedSizeBytes);

        FUniverseByteReader Reader(Writer.GetBytes());
        FUniverseSystemId Restored;
        UTEST_TRUE(Result, Restored.Deserialize(Reader));
        UTEST_TRUE(Result, Restored == A);
        UTEST_EQ_UINT(Result, Restored.Hash, A.Hash);
...
bool UniverseTest_SystemPhysicalPlausibility(FUniverseTestResult& Result)
{
    ...
    for (int64 X = 0; X < 30 && SystemsChecked < 200; ++X)
    {
        for (int64 Y = 0; Y < 30 && SystemsChecked < 200; ++Y)
        {
            ...
            for (int32 Index = 0; Index < Count; ++Index)
            {
                ...
                // Star.
                UTEST_TRUE(Result, System.Star.MassSolar > 0.05 && System.Star.MassSolar < 100.0);
                UTEST_TRUE(Result, System.Star.RadiusMeters > 1.0e7 && System.Star.RadiusMeters < 1.0e12);
                UTEST_TRUE(Result, System.Star.LuminositySolar > 0.0);
                UTEST_TRUE(Result, FMath::IsFinite(System.Star.LuminositySolar));
                UTEST_TRUE(Result, System.Star.SurfaceTemperatureK > 1000.0
                                && System.Star.SurfaceTemperatureK < 60000.0);
                UTEST_TRUE(Result, System.Star.ColorR >= 0.0 && System.Star.ColorR <= 1.0);
                UTEST_TRUE(Result, System.Star.ColorG >= 0.0 && System.Star.ColorG <= 1.0);
                UTEST_TRUE(Result, System.Star.ColorB >= 0.0 && System.Star.ColorB <= 1.0);
                UTEST_FALSE(Result, System.Star.Name.IsEmpty());
                ...
                UTEST_EQ_INT(Result, ReportedX, X);
                UTEST_EQ_INT(Result, ReportedY, Y);
                UTEST_EQ_INT(Result, ReportedZ, 0);
                UTEST_TRUE(Result, System.Position.IsNormalized());
                ...
                for (const FPlanetDescriptor& Planet : System.Planets)
                {
                    ...
                    UTEST_TRUE(Result, Planet.RadiusMeters > 1.0e5 && Planet.RadiusMeters < 1.0e9);
                    UTEST_TRUE(Result, Planet.MassKg > 0.0 && FMath::IsFinite(Planet.MassKg));
                    UTEST_TRUE(Result, Planet.SurfaceGravityMs2 > 0.0 && Planet.SurfaceGravityMs2 < 200.0);
                    UTEST_TRUE(Result, Planet.OrbitalPeriodSeconds > 0.0
                                    && FMath::IsFinite(Planet.OrbitalPeriodSeconds));
                    UTEST_TRUE(Result, Planet.EquilibriumTemperatureK > 0.0
                                    && Planet.EquilibriumTemperatureK < 20000.0);
                    UTEST_TRUE(Result, Planet.OrbitPhaseRadians >= 0.0
                                    && Planet.OrbitPhaseRadians <= 6.2832);
                    UTEST_TRUE(Result, FMath::Abs(Planet.OrbitInclinationRadians) < 0.5);
                    UTEST_TRUE(Result, FMath::Abs(Planet.RotationPeriodSeconds) > 3600.0);
                    UTEST_FALSE(Result, Planet.Name.IsEmpty());

                    // Orbits must be strictly increasing outward, and never
                    // inside the star itself.
                    UTEST_TRUE(Result, Planet.OrbitRadiusMeters > PreviousOrbit);
                    UTEST_TRUE(Result, Planet.OrbitRadiusMeters > System.Star.RadiusMeters);
                    ...
                    {
                        UTEST_TRUE(Result, Planet.RadiusMeters > 4.0e7);
                        UTEST_TRUE(Result, Planet.bHasAtmosphere);
                    }
                    if (Planet.Type == EPlanetType::Rocky || Planet.Type == EPlanetType::Terrestrial)
                    {
                        UTEST_TRUE(Result, Planet.RadiusMeters < 2.0e7);
                    }
                }
            }
        }
    ...
    UTEST_TRUE(Result, SystemsChecked > 20);
    UTEST_TRUE(Result, PlanetsChecked > 50);
        ...
        for (int64 Y = 0; Y < 30 && PlanetsChecked < 120; ++Y)
        {
            ...
            for (int32 Index = 0; Index < Count; ++Index)
            {
                ...
                for (const FPlanetDescriptor& Planet : System.Planets)
                {
                    ...
                    UTEST_TRUE(Result, A == B);
                    UTEST_TRUE(Result, A.IsNormalized());
                    ...
                    UTEST_TRUE(Result, ActualMeters > PreviousDistance);
...
bool UniverseTest_SectorPopulationStatistics(FUniverseTestResult& Result)
{
    ...
    for (int64 X = -10; X < 10; ++X)
    {
        for (int64 Y = -10; Y < 10; ++Y)
        {
            for (int64 Z = -10; Z < 10; ++Z)
            {
                ...
                UTEST_TRUE(Result, Count >= 0 && Count <= 2);
                TotalSystems += Count;
                ++TotalSectors;
    ...
    UTEST_NEAR(Result, Mean, 0.47, 0.05);
    UTEST_EQ_INT(Result, MaxInAnySector, 2);
    ...
    UTEST_TRUE(Result, DensityPerLy3 > 0.002 && DensityPerLy3 < 0.008);
...
bool UniverseTest_ProximityQueryDeterminism(FUniverseTestResult& Result)
{
    ...
    UTEST_TRUE(Result, FirstPass.Num() > 0);

    TArray<FStarSystemDescriptor> SecondPass;
    FStarSystemGenerator::FindSystemsWithin(Hierarchy, Centre, 12.0, SecondPass, 64);

    UTEST_EQ_INT(Result, SecondPass.Num(), FirstPass.Num());
    for (int32 Index = 0; Index < FirstPass.Num() && Index < SecondPass.Num(); ++Index)
    {
        UTEST_TRUE(Result, SecondPass[Index].Id == FirstPass[Index].Id);
        UTEST_EQ_UINT(Result, SecondPass[Index].GetContentHash(), FirstPass[Index].GetContentHash());
    }
    ...
    UTEST_TRUE(Result, Wider.Num() >= FirstPass.Num());
    ...
    if (FStarSystemGenerator::FindNearestSystem(Hierarchy, Centre, 20.0, Nearest))
    {
        ...
        UTEST_TRUE(Result, Nearest.Id == BestId);
    }
...
bool UniverseTest_LeaveAndReturnReproduction(FUniverseTestResult& Result)
{
    ...
    int64 HomeY = 0;
    int64 HomeZ = 0;
    UTEST_TRUE(Result, FindPopulatedSector(Hierarchy, HomeX, HomeY, HomeZ));

    FStarSystemDescriptor Home;
    UTEST_TRUE(Result, FStarSystemGenerator::GenerateSystem(Hierarchy, HomeX, HomeY, HomeZ, 0, Home));
    ...
    for (int32 Leg = 0; Leg < 50; ++Leg)
    {
        ...
        UTEST_TRUE(Result, Traveller.TryOffsetByCells(
    ...
    UTEST_TRUE(Result, TravelledLy > 500.0);
    ...
    UTEST_TRUE(Result, Traveller == HomePosition);

    // Regenerate the home system from its address alone.
    FStarSystemDescriptor Returned;
    UTEST_TRUE(Result, FStarSystemGenerator::GenerateSystem(Hierarchy, HomeX, HomeY, HomeZ, 0, Returned));

    UTEST_EQ_UINT(Result, Returned.GetContentHash(), HomeContentHash);
    UTEST_TRUE(Result, Returned.Position == HomePosition);
    UTEST_TRUE(Result, Returned.Name == HomeName);
    UTEST_EQ_INT(Result, Returned.Planets.Num(), HomePlanetCount);
    ...
    // gives the same result too.
    {
        ...
        UTEST_TRUE(Result, RestoredId.Deserialize(Reader));

        FStarSystemDescriptor FromSavedId;
        UTEST_TRUE(Result, FStarSystemGenerator::GenerateSystem(Hierarchy, RestoredId, FromSavedId));
        UTEST_EQ_UINT(Result, FromSavedId.GetContentHash(), HomeContentHash);
    }
