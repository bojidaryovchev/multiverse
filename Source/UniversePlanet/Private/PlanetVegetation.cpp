    ...
    bool& OutbHitBudget)
{
    ...
    // Thin uniformly rather than truncate - and thin *without moving the grid*.
    //
    // At most one instance is placed per cell, so a grid of R x R cells can
    // never exceed R^2 instances. The obvious way to respect a budget is
    // therefore to shrink the grid, and it is wrong: the grid cell is what
    // gives an instance its persistent name, so a smaller grid renames every
    // tree on the planet. A removal recorded by one caller then fails to match
    // anything generated by another with a different budget - which is exactly
    // the bug this comment replaced, found when a chopped tree came back after
    // a restart because the chop used a budget of 4000 and the streamer 2000.
    //
    // So the grid is a function of the patch and the biome table alone, and the
    // budget is applied as a per-cell keep roll instead. The cells stay where
    // they are, names stay stable, and a budget-limited patch is uniformly
    // sparser rather than dense in one corner and empty in the rest.
    //
    ...
    const double KeepFraction = (CellCount > MaxInstances)
        ? static_cast<double>(MaxInstances) / static_cast<double>(CellCount)
        : 1.0;
    ...
    for (int32 CellY = 0; CellY < Resolution; ++CellY)
    {
        for (int32 CellX = 0; CellX < Resolution; ++CellX)
        {
            ...
            // The budget thinning roll. Its own hash stream, so changing the
            // budget cannot disturb the jitter, the density roll or the
            // archetype choice - only whether this cell participates at all.
            if (KeepFraction < 1.0
                && UnitFromHash(UniverseHash::Hash(CellSeed, 0x4Bu, 1)) >= KeepFraction)
            {
                continue;
            }
